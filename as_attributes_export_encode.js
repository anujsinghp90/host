...
window.as_attributes_export = window.as_attributes_export || {
    ...
    download: function() {
        var that = this;
        var csv = "Scope,Name,ID,Type,Description\n";
        Object.keys(that.message.data.attributes.visitor).sort().forEach(function(key) {
            csv += "Visitor," + key + "," + that.message.data.attributes.visitor[key].id + "," + that.message.data.attributes.visitor[key].fullyQualifiedId + "," + that.message.data.attributes.visitor[key].description + "\n";
        });
        Object.keys(that.message.data.attributes.visit).sort().forEach(function(key) {
            csv += "Visit," + key + "," + that.message.data.attributes.visit[key].id + "," + that.message.data.attributes.visit[key].fullyQualifiedId + "," + that.message.data.attributes.visit[key].description + "\n";
        });
        ...
        gApp.inMemoryModels.quantifierCollection.models.forEach(function(key) {
            if (key.attributes.context.displayName.match(/visit/i) && !key.attributes.hidden) {
                that.message.data.attributes[key.attributes.context.displayName][key.attributes.name] = {
                    ...
                    description: encodeURI(key.attributes.description),
